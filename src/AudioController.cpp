...
void AudioController::init()
{
  ...
  for (uint8_t i = 0; i < MetronomeState::CHANNEL_COUNT; i++) {
    if (i == 0) { // Main beat channel - sharper, more prominent
    ...
    } else { // Secondary beat channel - softer, more tonal
...
float AudioController::applyEnvelope(ChannelSound &sound, uint32_t currentTime) {
  ...
  if (elapsedTime < sound.envelope.attackTime) {
    ...
    float releaseProgress = (float)(elapsedTime - (sound.duration - sound.envelope.releaseTime)) 
                           / sound.envelope.releaseTime;
...
void AudioController::processBeat(uint8_t channel, BeatState beatState) {
    ...
    if (beatState == ACCENT || beatState == WEAK) {
        ...
        channelSounds[channel].duration = (beatState == ACCENT) ? 
            (soundDurationMs * 1.5) : soundDurationMs;
    }
}
