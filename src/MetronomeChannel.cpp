...
MetronomeChannel::MetronomeChannel(uint8_t channelId) : id(channelId) {}

void MetronomeChannel::update(uint32_t globalBpm, uint32_t currentTime)
{
  if (!enabled)
    return;
...
uint8_t MetronomeChannel::getId() const
{
  return id;
}

uint8_t MetronomeChannel::getBarLength() const
{
  return barLength;
}

uint16_t MetronomeChannel::getPattern() const
{
  return pattern;
}

float MetronomeChannel::getMultiplier() const
{
  return multiplier;
}

uint8_t MetronomeChannel::getCurrentBeat() const
{
  return currentBeat;
}

bool MetronomeChannel::isEnabled() const
{
  return enabled;
}

bool MetronomeChannel::isEditing() const
{
  return editing;
}

uint8_t MetronomeChannel::getEditStep() const
{
  return editStep;
}
...
void MetronomeChannel::setMultiplier(float mult)
{
  multiplier = mult;
}
...
void MetronomeChannel::setEditing(bool edit)
{
  editing = edit;
}
...
void MetronomeChannel::updateProgress(uint32_t currentTime, uint32_t lastBeatTime, uint32_t globalBpm)
{
  if (!enabled)
    return;
...
void MetronomeChannel::updateBeat()
{
  if (!enabled)
    return;
  currentBeat = (currentBeat + 1) % barLength;
